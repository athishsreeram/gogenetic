...
func GoLangArch(conf data.AutoGeneratedCli, archType string, templateFile string) {
	...
	if archType == "es-sync-with-cqrs" {
		...
		conf.Gogenetic.TomlDir = conf.Gogenetic.Dir + "/toml/eventstore-read.toml"
...
func runGRPC(conf data.AutoGeneratedCli) {
	...
	runSwagger(conf, "proto")

}

func runSwagger(conf data.AutoGeneratedCli, dir string) {
	grpcswaggerargs := []string{"--proto_path=proto", "-I/usr/local/include", "-I.", "-I" + os.Getenv("GOPATH") + "/src", "-I" + os.Getenv("GOPATH") + "/src/github.com/grpc-ecosystem/grpc-gateway/third_party/googleapis", "--swagger_out=logtostderr=true:" + dir, conf.Gogenetic.Apiname + "-service.proto"}
