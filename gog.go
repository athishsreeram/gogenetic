...
func main() {
	...
	if archType == "file" {
		...
		initializeConf(box, conf, conf.Gogenetic.Lang+"/"+conf.Gogenetic.Lang+"-arch-config.toml", conf.Gogenetic.Lang+"/"+conf.Gogenetic.Lang+"-file-config.toml", archType)
	}
	...
	if dbType != "" {
		initializeConf(box, conf, "liquibase/liquibase-arch-config.toml", "liquibase/liquibase-file-config.toml", dbType)
		arch.LiquibaseLangArchConf(conf, dbType)
	}

	//Create Docker Compose
	initializeConf(box, conf, "docker/docker-arch-config.toml", "docker/docker-file-config.toml", "dockercomp")
	arch.DockerLangArchConf(conf, "dockercompose")

}

func initializeConf(box packr.Box, conf data.AutoGeneratedCli, archConfig string, utilConfig string, archType string) {
